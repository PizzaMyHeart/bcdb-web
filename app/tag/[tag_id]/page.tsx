...
export default async function allArticlesWithTag({ params }) {
    ...
    const articles = await prisma.$queryRaw`SELECT * FROM (SELECT articles.title, articles.is_closed_for_comments, articles.id AS article_id, articles.permalink AS article_permalink, articles.published_date, tags.name AS tag_name, tags.id AS tag_id, tags.permalink AS tag_permalink, articles.num_comments AS num_comments FROM articles_tags JOIN articles ON articles.id = articles_tags.article_id JOIN tags ON tags.id = articles_tags.tag_id) AS tagged WHERE num_comments IS NOT NULL AND tag_id=${tag_id};`;
